...
import {getColor, fitBounds, getYields, makeForecasts} from './lib/fields'
...
class App extends Component {
    ...
    render() {
        const {width, height, center, zoom, plantedFields, selectedField, fields, yields, forecasts, greatestTotalYield} = this.state;
        const totalYield = Object.keys(yields).reduce((totalYield, fieldName) => totalYield + yields[fieldName], 0).toFixed(2);
        const greatestYield = greatestTotalYield.toFixed(2);
                ...
                <LayersControl position="bottomright" collapsed={true}>
                    ...
                    <SimpleComponent position="topright" className="leaflet-control-actions">
                        <div>
                            <h2>{farm.name}</h2>
                            <div>
                                Your total yield is {totalYield} tons
                            </div>
                            <div>
                                Your greatest possible yield is {greatestYield} tons
                            </div>
                            ...
                            return (
                                <Overlay name={field.name} key={field.name} checked={true}>
                                    ...
                                    >
                                        <Tooltip
                                            permanent
                                            key={`tooltip${field.name}`}
                                            direction="center"
                                            className="leaflet-tooltip-field"
                                        >
                                            <h3>
                                                {yields[field.name] ? yields[field.name].toFixed(2) : null}
                                            </h3>
                                        </Tooltip>
                                        <Popup
                                            key={`popup${field.name}`}
                                        >
                                            <div>
                                                <h3>
                                                    {field.name}
                                                ...
                                                { plantedFields[field.name] ?
                                                    <div className="crops--selected">
                                                        <div className="crops-item">
                                                            <button className="crops-item-button"
                                                                    onClick={() => this.onRemoveCrop(field)}>
                                                        ...
                                                        return (
                                                            <div className="crops-item" key={crop.name}>
                                                                <button className="crops-item-button"
                                                                    ...
                                                                    {`Get ${possibleYield.toFixed(2)} tons`}
                                                                </button>
